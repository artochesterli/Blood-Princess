...
namespace PCG
{
	...
	public class Room
	{
			...
			for (int i = 0; i < entireRoomFile.Length; i++)
			{
				for (int j = 0; j < entireRoomFile[0].Length; j++)
				{
					...
					if (!Utility.IgnorePlacingStrHashSet.Contains(curChar))
						m_CurrentBoard[curTileWorldPosition.x, curTileWorldPosition.y] = entireRoomFile[i][j] + ";" + m_RoomMaker;
				}
			}
		}
		...
		private void _placeExpandableRoom(string roomType, IntVector2 startWorldPosition)
		{
			...
			string expandableRoomType = _getExpandableRoomTypeMarker(expandableroom, roomType);
			for (int i = 0; i < expandableroom.Length; i++)
			{
				for (int j = 0; j < expandableroom[i].Length; j++)
				{
					...
					if (!Utility.IgnorePlacingStrHashSet.Contains(curChar))
					{
						m_CurrentBoard[curTileWorldPosition.x, curTileWorldPosition.y] = expandableroom[i][j] + ";" + expandableRoomType;
					}
				}
			}
		}

		private string _getExpandableRoomTypeMarker(string[][] expandableroom, string roomType)
		{
			for (int i = 0; i < expandableroom[i].Length; i++)
			{
				for (int j = 0; j < expandableroom[i].Length; j++)
				{
					string curChar = expandableroom[i][j];
					if (Utility.RoomTypeMarkerHashSet.Contains(curChar))
					{
						return curChar;
					}
				}
			}
			Debug.Assert(false, "This expandable room has no room type attached: " + roomType);
			return "";
		}
		...
		private void _findExitPosition()
		{
			for (int i = 0; i < entireRoomFile.Length; i++)
			{
				for (int j = 0; j < entireRoomFile[i].Length; j++)
				{
					...
					_findRoomTypeMarker(i, j);
				}
			}
			Debug.Assert(m_RoomMaker != "", "Cannot find room marker for room" + _roomType);
		}

		/// <summary>
		/// Find the room's type marker
		/// </summary>
		/// <param name="x"></param>
		/// <param name="y"></param>
		private void _findRoomTypeMarker(int x, int y)
		{
			if (Utility.RoomTypeMarkerHashSet.Contains(entireRoomFile[x][y]))
			{
				m_RoomMaker = entireRoomFile[x][y];
			}
		}
